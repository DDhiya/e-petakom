...
<center>
		 <h1 style="text-align: center;">Bulletin List</h1>
       </br>
			<table id="bulletin">
			<tr>
				<br><br>
				<th>Bulletin ID</th>
        <th>Bulletin Image</th>
				<th>Title</th>
        <th>Description</th>
				<th>Action</th>
			</tr> 
   ...
   <tr>
            <td>&nbsp&nbsp{{$data->bulletinID}}</td>
            <!-- upload image -->
            <!-- <td> -->
            <!-- <img src="{{ asset($data->bulletinImage) }}" width= '60' height='60' class="img img-responsive" /> -->
            <!-- </td> ->>
            <!-- end upload image -->
            <td>&nbsp&nbsp{{$data->bulletinImage}}</td>
            <td>&nbsp&nbsp{{$data->title}}</td>
            <td>&nbsp&nbsp{{$data->description}}</td>
			<td><a href="click_edit/{{$data->bulletinID}}" class="btn btn-success">Edit</a><a href="click_delete/{{$data->bulletinID}}" class="btn btn-danger">Delete</a></td>
    </tr>
     @endforeach
		</table><br>
    <a href="AddBulletin" class="btn btn-success">Add Bulletin<class="nav-link  @if(url()->current() ===  URL::to('/AddReport'))
	active
    @endif">
	</a></center>
</div>
